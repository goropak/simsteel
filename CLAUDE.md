...
## 자동 진행 원칙 (묻지 말고 수행 — push만 사람 승인)

안전한 작업은 사람에게 "진행할까요?"라고 묻지 말고 바로 수행하고 결과만 보고한다. 사람 승인이 필요한 건 외부에 남거나 되돌리기 어려운 것뿐이다.

- **자동 수행(묻지 않음)**: 파일 읽기·편집·생성, `STATUS.md` 갱신, inbox 기록, 테스트/검증 증거 보존(기본 보존), `git add`·`git commit`.
- **사람 승인 필요(멈춤)**: `git push`·배포·파일 삭제 등 외부에 남거나 되돌리기 어려운 것 → `../approvals/`에 요청서(무엇/왜/대상/명령)를 떨궈 텔레그램 게이트로 올린다. ✅ 시 게이트가 자동 실행.
- 종료 의식(제6조)에서도 안전 단계는 묻지 않고 수행한다 — 멈추는 지점은 push 한 곳뿐이다.
