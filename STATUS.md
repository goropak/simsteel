...
## 💡 Why (존재 이유)
수백억 규모 제철소 투자 기획 시, 설비 배치를 머릿속이 아닌 화면에서 시뮬레이션해 공간 판단력을 높이기 위해.

## 🎯 Destination (완성 모습)
도면 위에 시설을 격자로 배치하면 동선·충돌·면적을 즉시 확인할 수 있는, 실무 투자 기획 회의에서 실제로 꺼내 쓰는 도구.

## 🗺️ Stages (여정 단계)
> ✅ 완료 / ← 현재 / (대기)
- Stage 1: 기본 격자 + 시설 배치 + 레이아웃 저장 ✅
- Stage 2: 서버 저장 + 개인 계정 + 이미지 Storage ✅ ← 현재
- Stage 3: 실무 적용 (실제 투자 기획 프로젝트에서 사용)
- Stage 4: 동선 분석 + 공간 최적화 제안 (Destination 도달)
