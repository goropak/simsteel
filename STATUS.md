...
## Backlog 추가 (2026-05-30, 사용자 요청)

### v0.2.6 후보 (선행): 공개 자료 도면 오버레이 프로토타입
- 캔버스 배경에 도면 이미지 반투명 표시 (opacity 슬라이더)
- 사용자가 시설을 도면 위에 클릭 배치 (수동 트레이싱)
- **사용 가능 자료**: TEFR/PFR Annexure의 공개 Plot Plan만
- **출처 메타**: `source: "JSW Utkal TEFR 2021"` 등 필수

### v0.4 (Supabase 도입 후): 사용자 자료 도면 오버레이 활성화
- **전제**: Supabase 인증 + Layer 2 격리 저장 구축 완료
- 사용자 업로드 도면 → Supabase 사용자별 스토리지로 직행
- 로컬 캐시 .gitignore 처리
- 외부 AI(GPT Vision 등)에 절대 전송 금지 (헌법 0조)
- 본인 계정에서만 보이고, 본인이 삭제 가능 (RLS)

### v0.5+: 표준 데이터 포맷 import
- JSW·DASTUR 등 협업 파트너가 simsteel JSON 포맷으로 직접 제공
- 도면 트레이싱 불필요, 한 번에 배치

### 폐기/보류: AI 비전 자동 인식
- GPT-4 Vision, Gemini Vision 등 외부 서비스 사용 금지 (헌법 0조)
- 사내 LLM 도입 시 재검토
